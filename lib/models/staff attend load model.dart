class StaffListResponse {
  final List<StaffUser> listData;

  StaffListResponse({required this.listData});

  factory StaffListResponse.fromJson(Map<String, dynamic> json) {
    final rawList = json['listData'];
    return StaffListResponse(
      listData: rawList is List
          ? rawList.map((e) => StaffUser.fromJson(e as Map<String, dynamic>)).toList()
          : <StaffUser>[],
    );
  }
...
class StaffUser {
  ...
  final StaffAttendance? staffAttendance; // ✅ proper model
  ...
  factory StaffUser.fromJson(Map<String, dynamic> json) {
    return StaffUser(
      ...
      // ✅ staffAttendance can be null OR object OR list -> safe
      staffAttendance: StaffAttendance.fromAny(json['teacherAttendance'] ?? json['staffAttendance']),
...
/// ✅ StaffAttendance model (safe even when API changes)
class StaffAttendance {
  ...
  static StaffAttendance? fromAny(dynamic value) {
    if (value == null) return null;

    if (value is Map<String, dynamic>) {
      return StaffAttendance.fromJson(value);
    }

    // If backend sends list, pick first item (common pattern)
    if (value is List && value.isNotEmpty && value.first is Map<String, dynamic>) {
      return StaffAttendance.fromJson(value.first as Map<String, dynamic>);
    }

    // Unknown format -> don't crash
    return StaffAttendance(extra: {'raw': value});
  }
