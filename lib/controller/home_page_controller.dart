...
// ✅ NEW: Recent enquiries (top 5 latest) for dashboard preview
  RxList<ViewEnquiryModel> latestEnquiryList = <ViewEnquiryModel>[].obs;
  RxBool isEnquiryLoading = false.obs;
  RxBool isEnquiryExpanded = false.obs;
  ...
  // ✅ NEW: Fetch top 5 latest view enquiries (pending or replied) for dashboard preview
  Future<void> fetchLatestEnquiries() async {
    try {
      isEnquiryLoading.value = true;
      final url = Uri.parse(
          '${AppUrl.base_url}api/EnquiryApp/ViewEnquiry1/${session.value}/${schoolid.value}');
      final response = await https.get(url);
      if (response.statusCode == 200) {
        final List<dynamic> data = jsonDecode(response.body);
        final list = viewEnquiryModelFromJson(data);
        list.sort((a, b) {
          final da = a.createDate ?? DateTime.fromMillisecondsSinceEpoch(0);
          final db = b.createDate ?? DateTime.fromMillisecondsSinceEpoch(0);
          return db.compareTo(da);
        });
        latestEnquiryList.assignAll(list.take(5).toList());
      } else {
        print("Failed to load enquiries, Status code: ${response.statusCode}");
      }
    } catch (e) {
