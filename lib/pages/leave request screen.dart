...
class _AddLeaveRequestTabState extends State<AddLeaveRequestTab> {
  ...
  void _openLeaveTypeSearchSheet(BuildContext context) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true, // 👈 lets the sheet grow/shrink with keyboard & content
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Remaining leave days — one chip per leave type
          Obx(() {
            if (controller.leaveBalanceList.isEmpty) return const SizedBox();
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemCount: controller.leaveBalanceList.length,
                    separatorBuilder: (_, __) => SizedBox(width: 10.w),
                    itemBuilder: (context, i) {
                      final bal = controller.leaveBalanceList[i];
                      return Container(
                        width: 120.w,
                        ...
                        child: FittedBox(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 4.h),
                              Text(
                                "${bal.remainingDays ?? 0} / ${bal.totalDays ?? 0} left",
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: TextStyle(
                                  fontSize: 12.sp,
...
class _ViewLeaveRequestTabState extends State<ViewLeaveRequestTab> {
  ...
  void initState() {
    ...
    // since fetchLeaveRequests() flips isLoading (Rx/Obx).
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Obx(() {
        ...
        final list = controller.leaveRequestList.toList();
...
class _LeaveTypeSearchField extends StatelessWidget {
  final LeaveTypeData? selected;
  final VoidCallback onTap;

  const _LeaveTypeSearchField({required this.selected, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: InputDecorator(
        decoration: InputDecoration(
          ...
          filled: true,
          fillColor: Colors.white,
          suffixIcon: Icon(Icons.search, color: axisMaroon.shade400, size: 20.sp),
        ),
        child: Text(
          selected?.leaveTypeName ?? "Search & select leave type",
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
          style: TextStyle(
            fontSize: 14.sp,
...
// Handles a large number of leave types cleanly:
//  - height is capped relative to screen size (responsive on small/large screens)
//  - the list itself always scrolls inside the sheet, so it never overflows
//  - a search box on top filters the list live
class _LeaveTypeSearchSheet extends StatefulWidget {
  final List<LeaveTypeData> options;
  final LeaveTypeData? selected;
  final ValueChanged<LeaveTypeData> onSelected;
...
class _LeaveTypeSearchSheetState extends State<_LeaveTypeSearchSheet> {
  ...
  late List<LeaveTypeData> _filtered;
  ...
  void _onSearchChanged(String query) {
    ...
    setState(() {
      _filtered = q.isEmpty
          ? widget.options
          : widget.options
          .where((e) => (e.leaveTypeName ?? '').toLowerCase().contains(q))
          .toList();
    });
  }
  ...
  @override
  Widget build(BuildContext context) {
    // Cap the sheet at ~70% of screen height so it stays responsive on
    // small phones and doesn't overflow on large lists — the ListView
    // below handles scrolling for whatever doesn't fit.
          ...
          child: SafeArea(
            ...
            child: Column(
              ...
              children: [
                ...
                // 👇 the scrollable, size-bounded list — works fine with a
                // handful of items or a very long list.
                Flexible(
                  child: _filtered.isEmpty
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final isSelected =
                          item.leaveTypeId == widget.selected?.leaveTypeId;
                      return ListTile(
                        dense: true,
                        title: Text(
                          item.leaveTypeName ?? '',
