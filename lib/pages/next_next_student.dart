// ============================================================
// FILE PATH: lib/pages/next_next_student.dart
// ============================================================
...
class Next2Student extends GetView<StudentController> {
  const Next2Student({super.key});
  ...
  Future<void> _pickImage(BuildContext context) async {
    final source = await showModalBottomSheet<ImageSource>(
      ...
      builder: (_) => _ImageSourceSheet(title: 'Student Photo'),
    );
    if (source == null) return;

    final XFile? picked = await controller.picker.pickImage(source: source);
    if (picked != null) {
      controller.fileImage.value = File(picked.path);
    } else {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            // Next button
            _NextButton(
              label: 'Next  →  Father Photo',
              onTap: () => Navigator.push(
                  context, MaterialPageRoute(builder: (_) => FatherImagePicker())),
            ),
...
// ══════════════════════════════════════════════════════════
// FATHER IMAGE PICKER  (father_image.dart)
// ══════════════════════════════════════════════════════════
// See father_image.dart

// ═══════════════════════════════════════════════════════════
// SHARED WIDGETS  (used across all image-picker screens)
// ═══════════════════════════════════════════════════════════

/// Step indicator row  e.g. ●●○○
