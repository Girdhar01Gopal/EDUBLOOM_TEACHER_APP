...
class _AppBarSubscriptionCircle extends StatelessWidget {
  final DashboardScreenController controller;
  const _AppBarSubscriptionCircle({required this.controller});

  int _daysRemaining(ExpiryData ed) {
    final today = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);
    final exp = DateTime(ed.expirydate.year, ed.expirydate.month, ed.expirydate.day);
    return exp.difference(today).inDays.clamp(0, 999999);
  }

  bool _isCritical(int days) => days <= 15;

  void _showDialogWithData(BuildContext context, ExpiryData ed) {
    final fmt        = DateFormat('dd MMM yyyy');
    final today      = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);
    final expiryOnly = DateTime(ed.expirydate.year, ed.expirydate.month, ed.expirydate.day);
    final daysRemaining   = expiryOnly.difference(today).inDays.clamp(0, 999999);
                    ...
                    Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          _SubDetailInfoRow(label: 'Plan Name', value: ed.planName ?? 'N/A', valueBold: true),
                          _SubDetailInfoRow(label: 'School ID', value: ed.schoolId, valueBold: true),
                          _SubDetailInfoRow(
                              label: 'Created On',
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final ed = controller.expiryInfo.value;
      ...
      final days = _daysRemaining(ed);
      final critical = _isCritical(days);
      final accentColor = critical ? Colors.red : const Color(0xFF6C63FF);

      return _BounceTap(
        onTap: () => _showDialogWithData(context, ed),
        child: Container(
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Text('days',
                      style: TextStyle(fontSize: 6.5.sp, color: Colors.grey.shade400)),
                ],
              ),
            ],
          ),
...
// same as the original card's _InfoRow — used inside the appbar dialog
class _SubDetailInfoRow extends StatelessWidget {
  final String label;
  final String value;
  final Color? valueColor;
  final bool valueBold;

  const _SubDetailInfoRow(
      {required this.label,
        required this.value,
        this.valueColor,
...
// used inside the appbar subscription details dialog — same visual as the
// mini info rows that used to show on the card's expand state
class _HeaderMiniInfoRow extends StatelessWidget {
  final IconData icon;
  final String label;
  final String value;
...
class Dhashoard extends GetView<DashboardScreenController> {
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(
      ...
      title: Obx(
            () => SizedBox(
              width: math.min(MediaQuery.of(context).size.width * 0.46, 220.w),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _ZoomableLogo(
      ...
      actions: [
        _AppBarSubscriptionCircle(controller: controller),
        SizedBox(width: 8.w),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
        ...
        onRefresh: () async {
          ...
          await widget.controller.fetchLatestEnquiries();
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _wrap(_op3, _sl3, _EnquiryPreviewSection(controller: widget.controller)),
...
class _EnquiryPreviewSection extends StatelessWidget {
  final DashboardScreenController controller;
  const _EnquiryPreviewSection({required this.controller});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      final pendingList = controller.latestEnquiryList
          .where((item) => !item.isReplied)
          .toList();

      if (!controller.isEnquiryLoading.value && pendingList.isEmpty) {
      ...
      return Column(
        ...
        children: [
          ...
          InkWell(
            borderRadius: BorderRadius.circular(30.r),
            onTap: () => controller.isEnquiryExpanded.toggle(),
            child: Row(
              children: [
                Container(
                  padding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 6.h),
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text("Recent Enquiries",
                          style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 0.4)),
                    ],
                  ),
                ),
                SizedBox(width: 12.w),
                ...
                SizedBox(width: 8.w),
                Obx(() => Icon(
                  controller.isEnquiryExpanded.value
          ...
          Obx(() {
            if (!controller.isEnquiryExpanded.value) {
              return const SizedBox.shrink();
            }
            ...
            return Column(
              children: [
                // ✅ Horizontal slider — same visual language as the birthday cards
                SizedBox(
                  height: 60.h,
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _SlideIn(
                        ...
                        child: Padding(
                          ...
                          child: _EnquiryMiniCard(
                            item: item,
                            onTap: () => _showEnquiryDetailDialog(context, item),
                          ),
                        ),
                      );
                    },
                ...
                SizedBox(height: 12.h),
                _BounceTap(
                  onTap: () => Get.toNamed(RouteName.parentenquirypage),
                  child: Container(
                    width: double.infinity,
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(14.r),
                      boxShadow: [BoxShadow(color: const Color(0xFF004D40).withOpacity(0.25), blurRadius: 12, offset: const Offset(0, 5))],
                    ),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Text("More", style: TextStyle(color: Colors.white, fontSize: 13.sp, fontWeight: FontWeight.w800)),
                        SizedBox(width: 6.w),
...
// ═══════════ Enquiry detail dialog (dashboard preview se) ═══════════
void _showEnquiryDetailDialog(BuildContext context, ViewEnquiryModel item) {
  final EnquiryController enquiryController = Get.isRegistered<EnquiryController>()
      ? Get.find<EnquiryController>()
      : Get.put(EnquiryController());

  final title = item.title ?? item.className ?? 'Enquiry';
  final statusColor = item.isReplied ? Colors.green : Colors.red;
      ...
      return AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Text(
              ...
              style: TextStyle(color: statusColor, fontWeight: FontWeight.w700, fontSize: 11.sp),
            ),
          ],
        ),
        content: SingleChildScrollView(
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(item.message ?? '', style: TextStyle(fontSize: 13.sp)),
              SizedBox(height: 10.h),
              if (item.studentName != null)
                Text('Student: ${item.studentName}', style: TextStyle(fontSize: 12.sp, color: Colors.grey)),
              if (item.className != null || item.section != null)
                Text('Class: ${item.className ?? '-'} ${item.section ?? ''}', style: TextStyle(fontSize: 12.sp, color: Colors.grey)),
              if (item.createDate != null)
                Text('Date: ${fmt.format(item.createDate!)}', style: TextStyle(fontSize: 12.sp, color: Colors.grey)),
              if (item.subject != null && item.subject!.toString().isNotEmpty)
                Text('Subject: ${item.subject}', style: TextStyle(fontSize: 12.sp, color: Colors.grey)),
              if (item.type != null && item.type!.toString().isNotEmpty)
                Text('Type: ${item.type}', style: TextStyle(fontSize: 12.sp, color: Colors.grey)),
              if (item.isReplied && item.reply != null && item.reply!.isNotEmpty) ...[
                const Divider(),
                Text('Reply: ${item.reply}',
                    style: TextStyle(fontSize: 13.sp, color: Colors.green, fontWeight: FontWeight.w600)),
              ],
            ],
          ),
        ),
          ...
          if (!item.isReplied)
            ElevatedButton.icon(
              onPressed: () {
                ...
                _openEnquiryReplyDialog(context, item, enquiryController);
              },
              style: ElevatedButton.styleFrom(backgroundColor: _DS.tealDark),
...
// ═══════════ Reply dialog (EnquiryScreen wala hi logic yaha bhi use hota hai) ═══════════
void _openEnquiryReplyDialog(
    BuildContext context, ViewEnquiryModel item, EnquiryController enquiryController) {
    ...
    builder: (dialogContext) {
      return AlertDialog(
        ...
        actions: [
          ...
          Obx(() {
            final sending = enquiryController.isReplying(item.id ?? 0);
            return ElevatedButton(
              onPressed: sending
                  ? null
                  : () async {
                final success = await enquiryController.sendReply(
                  enquiryId: item.id ?? 0,
                ...
                if (success) {
                  Navigator.of(dialogContext).pop();
                } else if (enquiryController.replyError.value.isNotEmpty) {
                  Get.snackbar('Error', enquiryController.replyError.value);
                }
              },
              child: sending
                  ? const SizedBox(
...
class _EnquiryMiniCard extends StatelessWidget {
  final ViewEnquiryModel item;
  final VoidCallback? onTap;
  const _EnquiryMiniCard({required this.item, this.onTap});

  Color get _statusColor => item.isReplied ? Colors.green : Colors.red;
    ...
    return _BounceTap(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(10.r),
          border: Border.all(color: _statusColor.withOpacity(0.6), width: 1),
          ...
          children: [
            ...
            Expanded(
              child: Text(
                ...
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: TextStyle(fontSize: 10.5.sp, fontWeight: FontWeight.w700, color: _DS.textDark),
              ),
            ),
          ],
        ),
